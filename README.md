...
같은 Wi-Fi 접속을 전제로 하지 않고 ngrok public URL로 공유합니다.

```bash
./scripts/start_ngrok.sh
```

ngrok 터미널에 표시되는 `Forwarding` URL을 다른 팀에 공유하면 됩니다. ngrok이 처음이면 먼저 ngrok 설치와 로그인 토큰 설정이 필요합니다.
...
GPT API Key와 로컬 Codex 호출은 사용하지 않습니다. 자연어 검색은 로컬 키워드 파서로 검색어를 만들고, JobKorea 검색 결과 HTML을 크롤링합니다.
