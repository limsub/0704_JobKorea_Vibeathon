...
- 채널 관리: 기본 카탈로그 채널 표시/숨김, 사용자 정의 검색 키워드 채널 추가/삭제를 브라우저에 저장합니다.
- 공고 원문 JSON 표시: 공고 카드는 정규화된 `job_profile`, `raw`, `slack_messages` 구조를 그대로 보여줍니다.
- 톤 조절: raw, business, friendly 3단계 슬라이더로 채널 메시지 문구를 바꿉니다.
- 이모지 분류: 관심 있음, 지원 후보, 패스, 연봉 좋음 상태를 공고별로 기록합니다.
...
- 왼쪽 Channels 목록은 `data/job_roles.json`의 카탈로그와 브라우저에 저장된 사용자 채널을 합쳐 구성합니다.
